<template>
  <div class="v_change_pwd g-flex-column">
    ...
    <div class="v-change-pwd-container">
      <div class="v-form">
        <!-- 旧密码 -->
        <div v-show="userInfo.passwordStatus == 1" class="v-form-item-box">
          <div class="v-form-item-box-title">
            <span>{{ i18n.oldPwdText }}</span>
          </div>
          <div class="v-form-item g-flex-align-center">
            <div class="v-form-item-middle-input">
              <input :type="pwdSeeeVal ? 'text' : 'password'" v-model="form.oldPassword"
                :placeholder="i18n.oldPwdPlaceholderText" />
            </div>
        ...
        <div class="v-form-item-box">
          <div class="v-form-item-box-title">
            <span>{{ i18n.newPwdText }}</span>
          </div>
          <div class="v-form-item g-flex-align-center">
            <div class="v-form-item-middle-input">
              <input :type="pwdSecondSeeeVal ? 'text' : 'password'" v-model="form.password"
                :placeholder="i18n.newPwdPlaceholderText" />
            </div>
        ...
        <div class="v-form-item-box">
          <div class="v-form-item-box-title">
            <span>{{ i18n.comfirmPwdText }}</span>
          </div>
          <div class="v-form-item g-flex-align-center">
            <div class="v-form-item-middle-input">
              <input :type="pwdThreeSeeeVal ? 'text' : 'password'" v-model="form.password2"
                :placeholder="i18n.confirmPwdPlaceholderText" />
            </div>
        ...
        <div @click="apiChangeLoginPwdHandel" class="v-submit-btn g-flex-align-center g-flex-justify-center">
          {{ i18n.submitBtnText }}
        </div>
      </div>
      <!-- <BindPhonesPop :list="areaList"  @emitPhoneItemClick="emitPhoneItemClick" ref="refBindPhonesPop"/> -->
    </div>
  </div>
</template>

<script setup>
import { apiGetUserInfo, apiChangeLoginPwd } from '@/utils/api.js'
...
// 修改资金密码
async function apiChangeLoginPwdHandel() {
  store.loadingShow = true
  const { success, data } = await apiChangeLoginPwd(form)
  if (!success) return
