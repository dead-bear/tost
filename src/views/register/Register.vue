  ...
  <div class="v_register g-flex-column">
    <div class="v-register-head g-flex-align-center">
      ...
      <div class="v-register-head-language g-flex-align-center">
        ...
        <div @click="$router.push({ name: 'languageset' })" class="v-register-head-language-img g-flex-justify-center g-flex-align-center">
    ...
    <div class="v-register-container">
      ...
      <div class="v-nav-list g-flex-align-center">
        ...
        <div v-show="store.system.RegMode.includes('3')" @click="typeClick(3)" :class="form.type == 3 ? 'active' : ''" class="v-nav-item">
          <span>{{ i18n.mobileRegText }}</span>
        </div>
        <div v-show="store.system.RegMode.includes('4')" @click="typeClick(4)" :class="form.type == 4 ? 'active' : ''" class="v-nav-item">
          <span>{{ i18n.emailRegText }}</span>
        </div>
      </div>
      <div class="v-form">
        ...
        <!-- 手机 -->
        <div v-show="form.type == 1 || form.type == 3" class="v-form-item g-flex-align-center">
        ...
        <!-- 邮箱 -->
        <div v-show="form.type == 2 || form.type == 4" class="v-form-item g-flex-align-center">
          <div class="v-form-item-left g-flex-align-center">
        ...
        <div v-show="store.system.TidSet != 0" class="v-form-item g-flex-align-center">
          ...
          <div class="v-form-item-middle-input">
            <input type="text" :disabled="tidLock" v-model="form.tid" :placeholder="i18n.inviteCodeBiTianPlaceholderText">
          </div>
        </div>


        <!-- 验证码 -->
        <div v-show="form.type != 0 && form.type != 3 && form.type != 4" class="v-form-item g-flex-align-center">
          <div class="v-form-item-left g-flex-align-center">
        ...
        <div v-show="form.type == 0 || form.type == 3 || form.type == 4" class="v-form-item g-flex-align-center">
          ...
          <div @click="getImgCode" class="v-form-item-middle-right g-flex-align-center">
...
let pwdSecondSeeeVal = ref(false)
  ...
  //type 1 手机 2邮箱 0用户名
  // RegMode数组 0用户名 1手机 2邮箱 3手机图片验证注册 4邮箱图片验证注册///type: Number(store.system.RegMode[0]),
...
function getParams() {
  ...
  if(tid) {
    ...
    let localTid = window.localStorage.getItem('tid')
    if(localTid){
      tidLock.value = true
      form.tid = localTid
    }
...
function getImgCode() {
  let gudingUrl = '/main/index/imgVertify?key='
  form.codeKey = new Date().getTime()
  let url = MODE == 'development' ? (`/api${gudingUrl}`) : gudingUrl
  codeImgUrl.value = url + form.codeKey;
}
